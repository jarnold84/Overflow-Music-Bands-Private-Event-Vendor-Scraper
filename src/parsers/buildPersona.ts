...
function classifyOrgType(obj: DomainContext | Lead): string | null {
    const people = obj.people ?? [];
    const roles = people.map(p => p.role?.toLowerCase() || '');
...
function estimateTeamSize(obj: DomainContext | Lead): string | null {
